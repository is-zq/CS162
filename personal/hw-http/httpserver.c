...
void serve_file(int fd, char* path) {
  ...
  char size_s[32];
  snprintf(size_s,32,"%ld",file_stat.st_size);
  http_send_header(fd, "Content-Length", size_s); // TODO: change this line too
  http_end_headers(fd);
  char buffer[128] = {0};
  int read_bytes;
  while((read_bytes = read(file_fd,buffer,128)) > 0)
  {
	  int write_bytes;
	  char* ptr = buffer;
	  while(read_bytes > 0)
	  {
		  write_bytes = write(fd,ptr,read_bytes);
...
void serve_directory(int fd, char* path) {
  ...
  while((entry = readdir(dirp)) != NULL)
  {
	  ...
	  {
		  char* new_path = (char*)malloc(strlen(path) + strlen("/index.html") + 1);
  ...
  if(flag == 0)
  {
	  ...
	  while((entry = readdir(dirp)) != NULL)
	  {
		  http_format_href(ref,path,entry->d_name);
...
struct SockPipe
{
	int s_sock;	//source socket
	int d_sock;	//destination socket
};

static void* sock_pipe_func(void* arg)
{
	struct SockPipe* sock_pipe = (struct SockPipe*)arg;
	int s_sock = sock_pipe->s_sock,d_sock = sock_pipe->d_sock;
	char* buffer = (char*)malloc(LIBHTTP_REQUEST_MAX_SIZE);
	int read_bytes;
	while(1)
	{
		memset(buffer,0,sizeof(buffer));
		if((read_bytes = read(s_sock,buffer,1024)) < 0)
		{
			free(buffer);
			free(sock_pipe);
			pthread_exit(0);
		...
		while(read_bytes > 0)
		{
  		    write_bytes = write(d_sock,ptr,read_bytes);
 		    if(write_bytes < 0)
			{
				free(buffer);
				free(sock_pipe);
...
void handle_proxy_request(int fd) {
  ...
  */
  ...
  struct SockPipe* c2s = (struct SockPipe*)malloc(sizeof(struct SockPipe));
  c2s->s_sock = fd;c2s->d_sock = target_fd;
  struct SockPipe* s2c = (struct SockPipe*)malloc(sizeof(struct SockPipe));
  s2c->s_sock = target_fd;s2c->d_sock = fd;
  pthread_create(&tid_1,NULL,sock_pipe_func,c2s);
  pthread_create(&tid_2,NULL,sock_pipe_func,s2c);
...
{
	struct ThreadServer* serv = (struct ThreadServer*)arg;
	(serv->request_handler)(serv->fd);
	free(serv);
	pthread_exit(0);
}
...
void serve_forever(int* socket_number, void (*request_handler)(int)) {
  ...
  *socket_number = socket(PF_INET, SOCK_STREAM, 0);
  ...
  while (1) {
    ...
    /* PART 6 BEGIN */
	pthread_t tid;
	struct ThreadServer* serv = (struct ThreadServer*)malloc(sizeof(struct ThreadServer));
	serv->fd = client_socket_number;
	serv->request_handler = request_handler;
	pthread_create(&tid,NULL,thread_server_func,serv);
